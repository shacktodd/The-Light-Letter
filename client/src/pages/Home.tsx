...
export default function Home() {
  ...
  return <div className="newsletter-shell">
    <header className="newsletter-nav"><Link href="/" className="newsletter-brand"><span className="brand-orbit" /><span>The Light Letter</span></Link><nav><a href="#latest">Latest issue</a><a href="#recents">Recents</a><a href="#method">How we work</a></nav><span className="newsletter-status"><span /> Publishing on a three-day rhythm</span></header>
    <main>
      <section className="newsletter-hero"><div className="newsletter-hero-copy"><p className="eyebrow hero-signature"><Sparkles size={13} /> A letter for curious readers</p><h1 className="letter-title"><span>The</span><em>Light Letter</em></h1><p className="hero-subtitle">AI-powered cross-domain synthesis as your friendly newsletter for ideas that connect.</p><p className="hero-support">Fresh ways to see the world, with the sources and uncertainty always within reach.</p><div className="hero-links"><a href="#latest" className="newsletter-button">Read the latest <ArrowDownRight size={16} /></a><a href="#method" className="text-link">See the method <ArrowUpRight size={15} /></a></div></div><div className="hero-issue-card"><div className="issue-card-top"><span>Issue 001</span><span>{formatIssueDate(foundingIssue.publishedAt)}</span></div><p className="issue-card-kicker">The founding edition</p><h2>When a record becomes something else.</h2><p>Three ways institutions made information that later acquired a second life.</p><Link href={`/issue/${foundingIssue.slug}`} className="issue-card-link">Open the edition <ArrowUpRight size={15} /></Link><div className="orbit-field" aria-hidden="true"><i /><i /><i /><b /></div></div></section>
      ...
      <section id="method" className="method-section"><div><p className="eyebrow"><FileSearch size={13} /> How the Letter earns attention</p><h2>Surprise is the invitation.<br />Truth is the constraint.</h2></div><div className="method-steps">{dates.map((step, index) => <article key={step.label}><span>0{index + 1}</span><h3>{step.label}</h3><p>{step.text}</p></article>)}</div></section>

      <section className="newsletter-footer"><div><p className="eyebrow"><CheckCircle2 size={13} /> The Light Letter</p><h2>Ideas worth slowing down for.</h2></div><a href="#latest" className="newsletter-button">Read issue 001 <ExternalLink size={16} /></a></section>
    </main>
  </div>;
}
