...
export default function Issue() {
  ...
  if (!source) return <div className="newsletter-shell issue-shell"><Link href="/" className="back-link"><ArrowLeft size={16} /> Back to The Light Letter</Link><h1>That edition is not in the public archive yet.</h1></div>;
  ...
  return <div className="newsletter-shell issue-shell"><header className="newsletter-nav"><Link href="/" className="newsletter-brand"><span className="brand-orbit" /><span>The Light Letter</span></Link><button onClick={() => navigator.clipboard?.writeText(window.location.href)} className="nav-share"><Share2 size={15} /> Share issue</button></header><main className="issue-main"><Link href="/" className="back-link"><ArrowLeft size={16} /> All editions</Link><p className="issue-kicker">Issue / {formatIssueDate(source.publishedAt)}</p><h1>{source.title}</h1><p className="issue-standfirst">{source.standfirst}</p>{source.editorNote && <aside className="editor-note"><span>From the desk</span>{source.editorNote}</aside>}<div className="story-stack">{insights.map((insight, index) => <InsightStory key={insight.title} insight={insight} index={index} />)}</div><section className="issue-closing"><p>Want the short version next time?</p><h2>One surprising idea, one practical implication, and the evidence trail to check it.</h2><Button className="newsletter-button" onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}>Read another edition <ArrowUpRight size={16} /></Button></section></main></div>;
}
